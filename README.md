...
- `bridge remote install --target SSH_ALIAS --binary bin/bridge-go-linux-amd64 --json`: upload a prebuilt Linux/amd64 CLI after backing up an existing binary, initialize a missing remote manifest, and run remote doctor. Build the binary with `make remote-binary`. The installer does not copy local credentials; a new host may still need its isolated Codex catalog and loopback SSH identity configured before doctor is ready.
...
./bin/bridge-go --manifest ~/.config/codex-cpa-bridge/bridge.toml remote install --target devbox --binary bin/bridge-go-linux-amd64 --json
./bin/bridge-go --manifest ~/.config/codex-cpa-bridge/bridge.toml remote sync --target devbox --json
```

`remote sync` without `--write` is a preview. Apply only after reviewing the report. A first-time host may still need its CPA catalog, credential source, and loopback SSH identity configured before `remote scan` is ready; `remote install` does not copy local secrets.
