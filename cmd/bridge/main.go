...
func run(argv []string) int {
	...
	case "setup":
		...
		if err := bridge.Setup(os.Stdout, m, bridge.SetupOptions{
			...
			GenerateBridgeKey: *generateBridgeKey,
			Start:             !*noStart,
		}); err != nil {
	...
	case "remote":
		...
		setupRemote := fs.Bool("setup", false, "configure the remote loopback SSH endpoint after installation")
		...
		if args[0] == "install" {
			report, err := bridge.InstallRemote(*target, *binary, bridge.RemoteInstallOptions{Setup: *setupRemote})
			if err != nil {
