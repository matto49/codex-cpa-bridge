...
The installer requires `sha256sum` on the Linux host and verifies the uploaded staging file before activation. A failed checksum leaves the existing remote CLI in place and removes the bridge-owned staging file. Replacements preserve a uniquely named binary backup.
