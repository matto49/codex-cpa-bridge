  ...
  return (
    <div className="view">
      ...
      <section className="sync-panel">
        <div className="sync-panel-head"><div><h2>Platform sync</h2><p>Preview changes before writing. Unrelated configurations are preserved.</p></div><button className="primary" onClick={onSync} disabled={busy || !plan}>{remoteTarget ? "Sync local + remote" : "Sync local platforms"}</button></div>
        {plan?.items.map((item) => <div className="sync-row" key={item.id}><strong>{item.id}</strong><span className={`platform-state ${item.action}`}>{item.action}</span><span>{item.detail}{item.add?.length ? ` · Add: ${item.add.join(", ")}` : ""}{item.remove?.length ? ` · Remove: ${item.remove.join(", ")}` : ""}</span></div>) ?? <p className="section-help">Sync preview unavailable. Check the desktop app and manifest.</p>}
